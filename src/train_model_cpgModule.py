...
def convert_coord_to_seq(data_array):
    ...
    for i in range(array_shape[0]):
        ...
        for j in range(int(array_shape[1]/3)):
            ...
            if start<0:
                extra=[0]*(0-start)
                one_list[4*num_base_extend*j:4*num_base_extend*(j+1)]=extra+dna_dict[chr_name][0:end]
            elif end >len(dna_dict[chr_name]):
                extra=[0]*(end-len(dna_dict[chr_name]))
                one_list[4*num_base_extend*j:4*num_base_extend*(j+1)]=dna_dict[chr_name][start:len(dna_dict[chr_name])]+extra
            else:
...
def train_network(g, num_epochs, num_steps = 1, batch_size = 50, verbose = True):
    ...
    tf.add_to_collection("rnn_outputs", g['rnn_outputs'])
        ...
        for epoch in range(num_epochs):
            ...
            for i in range(int(len_train/batch_size)): #each batch
                ...
                for j in range(1): #each num_steps
                    ...
                    feed_dict={g['x1']: data_train_x1[rand_sample,(j*num_steps*num_feature):((j+1)*num_steps*num_feature)], g['y1']: data_train_y1[rand_sample,(j*num_steps):((j+1)*num_steps)], g['y2']: data_train_y2[rand_sample,(j*num_steps):((j+1)*num_steps)]}
...
def build_multilayer_lstm_graph_with_dynamic_rnn(
   ...
   # 2 Fully connected layer for x1
    with tf.name_scope('x1_fc1'):
        W_x1_fc1 = weight_variable([num_feature, num_feat_full_connect_layer])
        b_x1_fc1 = bias_variable([num_feat_full_connect_layer])
        h_x1_fc1 = tf.nn.relu(tf.matmul(x1, W_x1_fc1) + b_x1_fc1)
    with tf.name_scope('x1_fc2'):
        W_x1_fc2 = weight_variable([num_feat_full_connect_layer, num_feat_full_connect_layer])
        b_x1_fc2 = bias_variable([num_feat_full_connect_layer])
        h_x1_fc2 = tf.nn.relu(tf.matmul(h_x1_fc1, W_x1_fc2) + b_x1_fc2)
    rnn_outputs=h_x1_fc2


   # 1 Fully connected layer for label1
    with tf.name_scope('fc1'):
        W_fc1 = weight_variable([num_feat_full_connect_layer, num_feat_full_connect_layer])
    ...
    with tf.name_scope('fc2'):
        W_fc2 = weight_variable([num_feat_full_connect_layer, num_feat_full_connect_layer])
