...
num_feat_full_connect_layer=150 ##300
...
def cnn(x):
  ...
  with tf.name_scope('conv1'):
    ...
    h_conv1 = tf.nn.relu(conv_x2(x_image, W_conv1) + b_conv1)
  ...
  with tf.name_scope('conv2'):
    ...
    h_conv2 = tf.nn.relu(conv_x2(h_pool1, W_conv2) + b_conv2)
...
def train_network(g, num_epochs, num_steps = 1, batch_size = 50, verbose = True):
    print('load data:')
    ...
    tf.add_to_collection("rnn_x1", g['rnn_x1'])
    tf.add_to_collection("cnn_x2", g['cnn_x2'])
        ...
        for epoch in range(num_epochs):
            ...
            for i in range(int(len_train/batch_size)): #each batch
                ...
                for j in range(1): #each num_steps
                    ...
                    feed_dict={g['rnn_x1']: rnn_outputs_pred, g['cnn_x2']: cnn_outputs_pred, g['y1']: data_train_y1[rand_sample,(j*num_steps):((j+1)*num_steps)], g['y2']: data_train_y2[rand_sample,(j*num_steps):((j+1)*num_steps)]}
            ...
            if epoch%10==0:
                ...
                for i in range(int(len_test/batch_size)): #each batch
                    ...
                    for j in range(1): #each num_steps
                        ...
                        feed_dict={g['rnn_x1']:rnn_outputs_pred ,g['cnn_x2']: cnn_outputs_pred, g['y1']: data_test_y1[sample,(j*num_steps):((j+1)*num_steps)], g['y2']: data_test_y2[sample,(j*num_steps):((j+1)*num_steps)]}
...
def build_multilayer_lstm_graph_with_dynamic_rnn(
    ...
    rnn_x1 = tf.placeholder(tf.float32, [batch_size, num_feature_rnn_output], name='rnn_input1_placeholder')
    cnn_x2 = tf.placeholder(tf.float32, [batch_size, num_feat_cnn_output], name='cnn_input2_placeholder')
    ...
    combine_outputs=tf.concat([rnn_x1, cnn_x2], 1) 


   # 1 Fully connected layer for label1
    with tf.name_scope('fc1'):
        W_fc1 = weight_variable([num_feature_rnn_output+num_feat_cnn_output, num_feat_full_connect_layer])
        b_fc1 = bias_variable([num_feat_full_connect_layer])
        h_fc1 = tf.nn.relu(tf.matmul(combine_outputs, W_fc1) + b_fc1)

    # 1 Fully connected layer for label2
    with tf.name_scope('fc2'):
        W_fc2 = weight_variable([num_feature_rnn_output+num_feat_cnn_output, num_feat_full_connect_layer])
        b_fc2 = bias_variable([num_feat_full_connect_layer])
        h_fc2 = tf.nn.relu(tf.matmul(combine_outputs, W_fc2) + b_fc2)
...
t = time.time()
train_network(g, num_epochs, num_steps , batch_size)
print("It took", time.time() - t, "seconds to train ",num_epochs, " epochs" )
